...
*/
class BoundSlider extends Widget{
    //Minimum value
	...
	@:isVar public var textField(get_textField, set_textField):InputText;
    ...
    */
	...
	@:noCompletion private function get_textField():InputText {
		return textField;
	}
	@:noCompletion private function set_textField(_field:InputText):InputText {
		textField = _field;
		textField.label.addEventListener("change", onUpdateTextField);
		
		return textField;
	}
	
	private function onUpdateTextField(e:Event):Void {
		var v:Float = Std.parseFloat(textField.text);
	...
	private function changeAction():Void {
		//trace("BoundSlider::changeAction");
		var intValue:Int = Std.int(value);
		if(textField != null && Std.parseFloat(textField.text) != intValue) {
			textField.text = Std.string(intValue);
